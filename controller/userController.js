...
const removeFromCart = async (req, res) => {
    try {
        ...
        const updatedCart = await Cart.findOneAndUpdate(
            ...
            { $pull: { items: { productId } }, $inc: { totalPrice: -productPrice } },
            { new: true }
        );
...
const loadCheckoutPage = async (req, res) => {
    try {
        ...
        const address = addressDoc ? addressDoc.addresses : [];
        console.log(address);
        res.render("checkout", { user, address, cart: cart || { totalPrice: 0 }, coupons });
    } catch (error) {
    ...
    try {
        ...
        await Promise.all(
            parsedOrderItems.map(async (item) => {
                ...
                const decrementQuantity = item.quantity > 0 ? item.quantity : 1;
                product.quantity = Math.max(product.quantity - decrementQuantity, 0);
...
const loadWallet = async (req, res) => {
    try {
        ...
        const totalPage = Math.ceil(wallet.history.length / limit);
...
const addToWallet = async (req, res) => {
    try {
        ...
        const options = {
            amount: amount * 100,
            currency: "INR",
            receipt: `wallet_order_${userId.slice(0, 10)}_${Date.now()}`,
            payment_capture: 1,
        };
