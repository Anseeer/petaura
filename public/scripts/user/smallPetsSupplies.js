...
// Load the filter state from the URL when the page loads
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Update the filterState from the URL
    ...
    // Apply the saved filter state to the UI
    if (filterState.search) {
      document.getElementById("search").value = filterState.search;
    }
  
    // Apply the active category state
    ...
    // Apply sorting (you can add logic to highlight the selected sort option)
    fetchProducts(1);
  });
  ...
  // Update the URL with the current filter state using History API
  function updateUrl() {
    ...
    history.replaceState(null, '', url); // Replace the current URL without reloading
  }
    ...
    products.forEach((product, index) => {
      const productDetails = `
        <div class="col-md-4 mb-4">
          <a href="/user/ProuctDetailsOfSmallPets?product=${product._id}" class="text-decoration-none">
            <div class="Ccard position-relative">
              <span onclick="event.stopPropagation();  addToWishlist(event, '${product._id}')">
